from flask import Flask, flash, jsonify, redirect, render_template,session,request,abort
...
#database資料
POSTGRES = {
    'user' : 'postgres', #你的user name
    'db' : 'postgres',#你的db名稱
    'passwd': 'a1234567890',#你的密碼
    'host': 'finalproj-database.c2vrh8vtr5mc.us-east-1.rds.amazonaws.com' #你的db位置
}

#連線db
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:a1234567890@finalproj-database.c2vrh8vtr5mc.us-east-1.rds.amazonaws.com:5432/postgres'

#連線linebot
line_bot_api = LineBotApi('UDEjLRk+VlGCGL6s6ILCjCMOKw080r9YMoLqCEJ/Mg61bShQsrPD0jAhqBj9emLJJfkRcR61RI4qhN13Stiq3LB3sydpyufXWlnlUxxdGIvyqu4YkoKLYKVXCySvfeAGlAm3kGRsy6/4PfgAzEWQYQdB04t89/1O/w1cDnyilFU=')
handler = WebhookHandler('4bf7c2f981f220e0bdcbb313d008ce3c')
user_id = 'Ub471f1c5ed6a0f456e97014e9e4581d0'#我的id(琛)
    ...
    #查看股票趨勢圖
        ...
        try:
            ...
            for i in range(30):
                ...
                llist.append(total)
                output_lines.append(f"{cur}: 總價值 {total:.2f}")  # 將日期和总价值添加到输出行中
        
            out = '\n'.join(output_lines) 
            flex_message = TextSendMessage(text=out,
                               quick_reply=QuickReply(items=[
                                   QuickReplyButton(action=MessageAction(label="回到開始選單", text="開始")),
                                   QuickReplyButton(action=MessageAction(label="新增投資組合", text="新增投資組合")),
                                   QuickReplyButton(action=MessageAction(label="刪除投資組合", text="刪除投資組合")),
        ...
        #1, "2023-12-31"
            ...
            flex_message = TextSendMessage(text="圖表查詢錯誤",
                               quick_reply=QuickReply(items=[
                                   QuickReplyButton(action=MessageAction(label="回到開始選單", text="開始")),
                               ]))
            line_bot_api.reply_message(event.reply_token, flex_message)
